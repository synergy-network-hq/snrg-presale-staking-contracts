...
contract ForkSourcesTest is Test {
    ...
    function testBaseActualFourSourceBoundaryInputs() external {
        ...
        assertGt(IERC20Metadata(BASE_UNLOCKED).balanceOf(unlockedHolder), 0, "unlocked holder empty");
        assertGt(IERC20Metadata(BASE_LOCKED).balanceOf(lockedHolder), 0, "locked holder empty");

        BaseSAFTVoucherAdapter adapter = new BaseSAFTVoucherAdapter(saft, saftDecimals);
        (address owner, uint256 entitlementNwei, bytes32 allocationId, bool consumed) = adapter.entitlement(saftTokenId);
        ...
        SynergyBaseStaking staking = new SynergyBaseStaking(
            address(this), BASE_UNLOCKED, BASE_LOCKED, address(adapter), uint64(block.timestamp), 0
        );

        // The production locked token deliberately permits transfers only through its one-time staking endpoint.
        // Simulate the exact Governance Safe configuration transaction required after the final staking address exists.
        vm.prank(GOVERNANCE_SAFE);
        (bool configured,) = BASE_LOCKED.call(abi.encodeWithSignature("setStaking(address)", address(staking)));
        assertTrue(configured, "locked token staking endpoint configuration failed");

        uint256 unlockedRaw = 1 ether;
        vm.startPrank(unlockedHolder);
        IERC20Metadata(BASE_UNLOCKED).approve(address(staking), unlockedRaw);
        uint256 unlockedPosition = staking.stakeUnlockedSNRG(unlockedRaw, StakingTerms.Term.THREE_MONTHS);
        vm.stopPrank();
        (address unlockedOwner, uint256 unlockedPrincipal) = _baseOwnerPrincipal(staking, unlockedPosition);
        ...
        vm.stopPrank();
        (address lockedOwner, uint256 lockedPrincipal) = _baseOwnerPrincipal(staking, lockedPosition);
        assertEq(lockedOwner, lockedHolder, "locked stake owner");
        assertEq(lockedPrincipal, 1e9, "locked normalized principal");

        vm.prank(owner);
        uint256 voucherPosition = staking.stakeBaseVoucher(
            saftTokenId, entitlementNwei, StakingTerms.Term.THREE_MONTHS
        );
        (address voucherStakeOwner, uint256 voucherPrincipal) = _baseOwnerPrincipal(staking, voucherPosition);
    ...
    function testEthereumActualPresaleVoucherABI() external {
        ...
        (address owner, uint256 entitlementNwei, bytes32 allocationId, bool consumed) = adapter.entitlement(tokenId);
        ...
        SynergyStakingRewardVoucher rewardLedger = new SynergyStakingRewardVoucher(address(this), "");
        ...
        vm.prank(owner);
        uint256 positionId = staking.stakeVoucher(tokenId, entitlementNwei, StakingTerms.Term.THREE_MONTHS);
        (address voucherStakeOwner, uint256 voucherPrincipal) = _ethereumOwnerPrincipal(staking, positionId);
        ...
        returns (address owner, uint256 principalNwei)
    {
        ...
        bytes32 rewardId;
        (
            , owner, source, sourceAsset, sourceTokenId, allocationId, rawPrincipal, principalNwei,
            rewardNwei, rewardBps, startedAt, maturesAt, status, rewardId
        ...
        returns (address owner, uint256 principalNwei)
    {
        ...
        bytes32 rewardId;
        (
            , owner, sourceTokenId, allocationId, principalNwei, rewardNwei, rewardBps,
            startedAt, maturesAt, status, rewardId
        ) = staking.positions(positionId);
    }
}
